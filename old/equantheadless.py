...
def nokia_print(newline):
    nokia_lines.append(newline)
    ...
    for idx, line in enumerate(nokia_lines[-5:]):
    #draw.text((1,1), 'Starting.', font=font)
        draw.text((1,idx*8), line, font=font)
    disp.image(image)
    disp.display()
def nokia_clear():
    nokia_lines = []
...
class HeadlessSynth:
    ...
    def load_bank(self, bfile):
        ...
        try:
            ...
            nokia_print(f"Error loading {bfile}\n{str(e)}")
            error_blink(3)
        print("Bank loaded.")
        nokia_print("Bank loaded.")
    ...
    def select_patch(self, n, force=False):
        ...
        nokia_print(f"Ptch: {n + 1}/{len(fp.patches)}:")
        nokia_print(f"{fp.patches[n]}")
    ...
    # catches custom midi :sig to change patch/bank
        ...
        if sig.type == 'prog':
            ...
            if pno < 0:
                ...
                self.select_patch(sig.par1)
            else:
                nokia_print(f"No pno {pno}!")
